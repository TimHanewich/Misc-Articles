...
The UART Rx buffer size differs between devices, but I've seen buffers listed as little as 32 bytes and larger than 99,999. If the buffer exceeds this length, old bytes will be lapsed and be "overwritten" with new bytes (continuous stream with a cutoff).
